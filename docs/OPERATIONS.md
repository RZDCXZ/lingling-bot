...
| 当前代码验收 | 类型检查、构建及获得本机临时端口权限后的 18 个测试文件共 109 项测试通过。Docker 核心已用 `restart:core` 实机重建，NapCat 容器保持不变；QQ 登录失效、3001 尚未监听时，核心会保持运行并后台重连。主动互动和新版延年益寿的真实群触发仍需观察 |
...
把它用作机器人登录命令。登录命令与 `status` 共用 OneBot `get_status` 探针，只有
`online=true` 且 `good=true` 才视为在线，不以 3001 端口是否可连接代替 QQ 登录态。
