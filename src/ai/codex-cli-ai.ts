  ...
  switch (mode) {
    ...
    case "daily-longevity":
      return {
        ...
        success:
          "审核标准从宽：最终消息必须明确出现“延年益寿”四个字。由你结合画面整体判断；普通群聊可以分享时原则上通过，包括常规二次元插画、动物图、萌系图、搞笑图和其他轻松内容。只有在明确出现真人私密或性化内容、明显未成年角色的性化呈现、裸体或关键部位裸露、性行为、强烈恋物特写、胁迫、暴力色情，或整体确实不适合普通群聊时才拒绝。不得仅因无法证明具体年龄、非女性、非二次元、单纯校服或萌系画风就自动拒绝；应根据实际画面是否包含明确风险来判断。逐张独立判断，只选择通过的序号；全部不合适就保持安静。配文一至两句，口语、俏皮，可以略带暧昧但不能露骨描述身体或性行为，也不要声称图片来自铃铃酱本人。",
        tools:
